...
export class MesasComponent implements OnInit {
  ...
  @ViewChild("validatorVotos", { static: false })
      ...
      (data) => {
        ...
        for (let i = 0; i < data.length; i++) {
          list.push({
            ...
            tipo:
              data[i].recinto.tipo.length == 1
                ? data[i].recinto.tipo[0]
                : data[i].recinto.tipo[0] + "/" + data[i].recinto.tipo[1],
  ...
  cellrender2 = (row: number, column: any, value: any): any => {
    ...
    if (sumvotos == 0 || value == 0 || sumvotos <= this.inemp.val())
      return (
        "<h6 style='height:100%;text-align:center;vertical-align:middle;'><div class='cell-default'>" +
        value +
        "</div></h6>"
      );
    else
      return (
        "<h6 style='height:100%;text-align:center;vertical-align:middle;'><div class='cell-red'>" +
        value +
        "</div></h6>"
      );
  };
  columns2: any[] = [
    { datafield: "candidatura", text: "", width: "19%", editable: false },
    {
      ...
      width: "9%",
      editable: true,
      cellsrenderer: this.cellrender2,
    },
    {
      datafield: "ADN",
      text: "ADN",
  ...
  estadorender = (row: number, column: any, value: any): any => {
    if (value == "Verificado")
      return (
        "<h6 style='height:100%;line-height:1.5;text-align:center;vertical-align:middle;'><span class='badge badge-success'>" +
        value +
        "</span></h6>"
      );
    else if (value == "Anulado")
      return (
        "<h6 style='height:100%;line-height:1.5;text-align:center;vertical-align:middle;'><span class='badge badge-warning'>" +
        value +
        "</span></h6>"
      );
    else
      return (
        "<h6 style='height:100%;line-height:1.5;text-align:center;vertical-align:middle;'><span class='badge badge-secondary'>" +
        value +
        "</span></h6>"
      );
  };
  ...
  columns: any[] = [
    ...
    { datafield: "mesa", text: "Mesa", width: 80 },
    {
      ...
      text: "estado",
      width: 110,
      cellsrenderer: this.estadorender,
    },
    {
      datafield: "circunscripcion",
      text: "Circuns.",
      width: 100,
  ...
  cirCambiando(event: any): void {
    ...
    if (this.toggle.val()) {
      console.log(
        "listo para filtrar por ",
        this.dropCir.getSelectedItem().value
      );
    }
  ...
  provCambiando(event: any): void {
    ...
    if (this.toggle.val()) {
      console.log(
        "listo para filtrar por ",
        this.dropCir.getSelectedItem().value,
        this.dropProv.getSelectedItem().value
      );
    }
  ...
  munCambiando(event: any) {
    ...
    if (this.toggle.val()) {
      console.log(
        ...
        this.dropCir.getSelectedItem().value,
        this.dropProv.getSelectedItem().value,
        this.dropMun.getSelectedItem().value
      );
    }
  }
  texto: any;
  filtrar() {
    if (this.toggle.val()) {
      ...
      if (cir != null) data.circunscripcionId = cir.value;
      if (pro != null) data.provinciaId = pro.value;
      if (mun != null) data.municipioId = mun.value;
      if (this.texto != undefined) data.institucion = this.texto;
      console.log(data);
    }
  }
  ...
  next() {
    ...
    if (rowselect != -1) {
      ...
      if (rowdata.estado === "Verificado") {
        this.mensaje(
          "Ya no se puede acceder porque ya esta verificada",
          "Votacion",
          1
        );
        return;
  ...
  downloadFoto() {
    this.$vot.downloadImage(this.acta.id).subscribe((response) => {
  ...
  processing(data: any, type: string) {
    ...
    if (!pwa || pwa.closed || typeof pwa.closed == "undefined") {
      this.mensaje(
        "Porfavor deshabilita tu Pop-up Blocker e intenta de nuevo",
        "",
        1
      );
    }
        ...
        (res) => {
          ...
          this.$vot.updateVotos(row._id._id, formVotos).subscribe(
            (res2) => {
              ...
              this.$spinner.hide();
              this.migrid.setcellvalue(
                this.migrid.getselectedrowindex(),
                "estado",
                "Verificado"
              );
            },
  ...
  checkVotos() {
    let rows = this.gridvot.getdisplayrows(),
      diff = 0,
      message = "";
    ...
    return message !== ""
      ? "Los votos en las Filas:  " +
          message +
          " deben ser menores a los empadronados."
      : null;
  }
  ...
  rules = [
    ...
    {
      input: ".inEmp",
      message: "debe estar entre 1 y 1024",
      action: "keyup, blur",
